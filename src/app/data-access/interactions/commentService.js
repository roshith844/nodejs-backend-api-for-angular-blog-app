...
async function addCommentToDatabase(blogId, userId, comment) {
    const RESPONSE = await POST_MODEL.updateOne({ _id: blogId }, { $push: { comments: { "userId": userId, "message": comment } } }, { new: true, upsert: true })
    if (RESPONSE.acknowledged === true) return RESPONSE
    return false
