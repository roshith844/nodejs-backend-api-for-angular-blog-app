...
async function incrementPageViewInDatabase(articleId) {
    const RESPONSE = await POST_MODEL.updateOne({ _id: articleId, deleted :  false  }, { $inc: { pageviews: 1 } })
...
async function getPageViewsFromDatabase(articleId) {
    const RESPONSE = await POST_MODEL.findOne({ _id: articleId , deleted :  false }, { pageviews: 1, _id: 0 })
    if (RESPONSE != null) {
